...
class Page(QWidget):
    ...
    def toggleUserAgent(self):
        ...
        self.browser.stop()
        if self.useragent == 'pc':
            self.browser.resize(self.browser.parent().frameGeometry().width(),
                                self.browser.parent().frameGeometry().height())
            self.browser.setMaximumSize(self.browser.parent().frameGeometry().width(),
                                        self.browser.parent().frameGeometry().height())
            self.browser.move(0, 0)
        elif self.useragent == 'mobile':
            self.browser.resize(int(self.browser.parent().frameGeometry().height() * 9 / 16),
                                self.browser.parent().frameGeometry().height())
            self.browser.setMaximumSize(int(self.browser.parent().frameGeometry().height() * 9 / 16),
                                        self.browser.parent().frameGeometry().height())
            self.browser.move(
                int(self.browser.parent().frameGeometry().width() / 2 - self.browser.frameGeometry().width() / 2), 0)
        self.browser.reload()
        self.toolbar['useragent'].setText('PC' if self.useragent == 'pc' else 'Mobile')
        self.toolbar['useragent'].setIcon(
            QIcon("assets/icons/useragent_pc.svg" if self.useragent == 'pc' else "assets/icons/useragent_mobile.svg"))
